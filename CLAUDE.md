...
- **D14 — Hosting: GitHub Pages** (revised; was Cloudflare Pages).
  Cloudflare is the better CDN, but connecting a repository to Cloudflare Pages requires steps
  in their dashboard that cannot be scripted, so the site could not go live without manual
  setup. GitHub Pages is configured entirely by `.github/workflows/deploy.yml`, so a push to
  main publishes with nothing to click. Both serve static HTML and the Flutter app on one
  domain with path routing, which is all the architecture needs. Moving to Cloudflare later is
  a DNS change plus a dashboard connection, not a rewrite.
- **D20 — SEO architecture.** Flutter Web renders into a WebGL canvas: no text nodes, no
  headings, no crawlable links, so it ranks for nothing. Since organic discovery is the whole
  distribution strategy, the indexable content is **hand-written semantic HTML**, generated by
  `tool/generate_seo.dart`, with the app mounted at `/play/`.
  Every technique diagram is an SVG produced by driving the **real human solver** until that
  technique fires — a hand-drawn one could quietly stop matching the game.
  The technique pages use **standalone prose**, not the in-app hint strings: those are written
  around live board coordinates, and stripping the placeholders yields "every cell of region …
  lies in this … …", which is filler.
- **D21 — Analytics: cookie-free, no identifiers.** Every event is a counter, bucketed so that
  nothing approaches a fingerprint (durations become `<1m`/`1-3m`/`3-10m`/`10m+`, progress
  becomes a quartile, group rather than puzzle id). The app calls a global `nodroTrack` hook in
  `web/index.html`; with no provider wired the calls are silent no-ops, because shipping
  without analytics must never break the game. The question it exists to answer is one:
